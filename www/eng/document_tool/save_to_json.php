...
// 최대 num 값 조회
// $result = mysqli_query($conn, "SELECT MAX(num) AS max_num FROM document_eng");
...
if (move_uploaded_file($_FILES['fileToUpload']['tmp_name'], $target_file_route)) {
    $insert_sql = "INSERT INTO document_eng (date, title, docuroute) 
    ...
    if (mysqli_query($conn, $insert_sql)) {
        ...
        if (mysqli_multi_query($conn, $reorder_sql)) {
            echo "<script>
                ...
                window.location.href = 'document_insert_eng.php';
                </script>";
        } else {
