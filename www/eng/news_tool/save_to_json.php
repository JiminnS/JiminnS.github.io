...
$title = $_POST['title'];
// TinyMCE에서 넘어온 comment를 그대로 저장 (HTML 형식 포함)
...
move_uploaded_file($_FILES['fileToUload']['tmp_name'], $target_file_route);

// $category에 따른 $category_link 설정
$category_link = $category; // 기본적으로 category와 동일하게 설정

if ($category === "NEC in the press") {
    $category_link = "news_media.php";
} else if ($category == "ASEAN News") {
    $category_link = "news_asean.php";
} else if ($category == "Environmental Info") {
...
$newFile_route = '../' . $newFileName;

// 새로운 기사를 데이터베이스에 저장
$sql = "INSERT INTO news_eng (date, category, title, head_img, content1, link, category_link) 
...
if (empty($finalContent)) {
    die("Template processing failed. Final content is empty.");
}

// 최종 PHP 파일 생성
file_put_contents($newFile_route, $finalContent);
...
echo "<script>
    ...
    window.location.href = 'news_insert_eng.php';
    </script>";
?>
